function [mean_ ,std_,max_,min_]= calc_composite_results_gold2(error_results,j_re_max,i_prob_max,i_step_max)
...
for j_field = 1:numel(names)
    for j_reg=1:j_re_max
        for i_prob=1:i_prob_max
            ...
            eval (['std_.',var_name{1},'(',num2str(j_reg),',',num2str(i_prob),',1) = std(squeeze(',...
                var_name{1},'(',num2str(j_reg),',',num2str(i_prob),',:,1)));'])
            
            eval (['std_.',var_name{1},'(',num2str(j_reg),',',num2str(i_prob),',2) = std(squeeze(',...
                var_name{1},'(',num2str(j_reg),',',num2str(i_prob),',:,2)));'])
            
            eval (['max_.',var_name{1},'(',num2str(j_reg),',',num2str(i_prob),',1) = max(squeeze(',...
                var_name{1},'(',num2str(j_reg),',',num2str(i_prob),',:,1)));'])
            
            eval (['max_.',var_name{1},'(',num2str(j_reg),',',num2str(i_prob),',2) = max(squeeze(',...
                var_name{1},'(',num2str(j_reg),',',num2str(i_prob),',:,2)));'])
            
            eval (['min_.',var_name{1},'(',num2str(j_reg),',',num2str(i_prob),',1) = min(squeeze(',...
                var_name{1},'(',num2str(j_reg),',',num2str(i_prob),',:,1)));'])
            
            eval (['min_.',var_name{1},'(',num2str(j_reg),',',num2str(i_prob),',2) = min(squeeze(',...
                var_name{1},'(',num2str(j_reg),',',num2str(i_prob),',:,2)));'])
        end
    end
end
