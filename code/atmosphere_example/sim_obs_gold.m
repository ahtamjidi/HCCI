...
opt_dist.sim.obs.z_cen{opt_dist.i_step} = [];
opt_dist.sim.obs.r_var_cen{opt_dist.i_step} = [];
opt_dist.sim.obs.id_vis_cen{opt_dist.i_step} = [];
...
for idx_agent=1:opt_dist.nAgents
    ...
    for i=idx_obs_agent
        ...
        switch opt_dist.FLAGS.obs_noise_type
            case 'absolute'
                r = opt_dist.obs.R;
            case 'relative'
                r = max(10^-5,opt_dist.obs.rel_perc.*z_temp);
        end
    ...
    opt_dist.sim.obs.z_cen{opt_dist.i_step} = [opt_dist.sim.obs.z_cen{opt_dist.i_step};z];
    opt_dist.sim.obs.r_var_cen{opt_dist.i_step} = [opt_dist.sim.obs.r_var_cen{opt_dist.i_step};r];
    opt_dist.sim.obs.id_vis_cen{opt_dist.i_step} = [opt_dist.sim.obs.id_vis_cen{opt_dist.i_step};id_vis];
